...
public:
                ...
                _In_ const std::string &per_object_stats_mode)
    {
        ...
        size_t maxCounterGroupSize = 0;
        size_t maxCounterGroupInd = 0;
        for ( size_t i = 0; i < m_supportedCounterGroups.size(); i++)
        {
            if (m_supportedCounterGroups[i].size() > maxCounterGroupSize)
            {
                maxCounterGroupSize = m_supportedCounterGroups[i].size();
                maxCounterGroupInd = i;
            }
        }
        std::vector<StatType> supportedIds(m_supportedCounterGroups[maxCounterGroupInd].begin(), m_supportedCounterGroups[maxCounterGroupInd].end());
        ...
        if (m_objectSupportedCountersGroupMap.find(vid) == m_objectSupportedCountersGroupMap.end())
{
            ...
            for (size_t i = 0; i < m_supportedCounterGroups.size(); i++)
            {
                if (m_supportedCounterGroups[i] == supportedIds)
                {
                    m_objectSupportedCountersGroupMap.emplace(vid, i);
                    groupAssigned = true;
                }
            }
