...
sai_object_id_t SyncdNvdaBfTest::CreateRouteGroup()
{
    SWSS_LOG_ENTER();

    sai_object_id_t oid;

    EXPECT_EQ(SAI_STATUS_SUCCESS, m_sairedis->create((sai_object_type_t)SAI_OBJECT_TYPE_OUTBOUND_ROUTING_GROUP, &oid, m_switchId, 0, {}));

    return oid;
}

void SyncdNvdaBfTest::RemoveRouteGroup(sai_object_id_t routeGroup)
{
    SWSS_LOG_ENTER();

    EXPECT_EQ(SAI_STATUS_SUCCESS, m_sairedis->remove((sai_object_type_t)SAI_OBJECT_TYPE_OUTBOUND_ROUTING_GROUP, routeGroup));
}
...
TEST_F(SyncdNvdaBfTest, dashOutboundRoutingEntry)
{
    ...
    sai_object_id_t counter = CreateCounter();
    sai_object_id_t vnet = CreateVnet(101);
    sai_object_id_t route_group = CreateRouteGroup();
    ...
    entry0.outbound_routing_group_id = route_group;
