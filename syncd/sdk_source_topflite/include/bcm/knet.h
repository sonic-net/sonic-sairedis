...
#define BCM_KNET_NETIF_F_KEEP_RX_TAG    0x00000004 /* Do nothing to the tag of
                                                      ...
                                                      to this interface. */
#define BCM_KNET_NETIF_F_HW_ROUTE       0x00000008 /* Hardware Router the packet */

#define BCM_KNET_NETIF_F_VXLAN_ENCAP    0x00000010 /* Encap with VxLAN Header. Valid for
                                                      l3 vni interfaces */

#define BCM_KNET_NETIF_F_NO_TX          0x00000020 /* disallow TX on this
                                                      interface. */
...
typedef struct bcm_knet_netif_s {
    ...
    uint16 dev_id;                      /* HPE PATCH: added to set the dev id
                                           during knet interface creation
                                           which linux uses along with mac
                                           address to generate link local
                                           IPv6 address */
...
#define BCM_KNET_DEST_T_CALLBACK    3          /* Send packet to kernel
                                                  call-back function. */
#define BCM_KNET_DEST_T_DYN_NETIF   4          /* HPE PATCH: Send packet to
                                                * port interface based on
                                                * ingress port.
                                                */
...
#define BCM_KNET_FILTER_F_STRIP_TAG 0x00000001 /* Strip VLAN tag from packets
                                                  ...
                                                  interfaces. */
#define BCM_KNET_FILTER_F_CON_STRIP_TAG 0x00000002 /* Conditionally strip VLAN
                                                      tag from packets sent to
                                                      virtual network interfaces. */
#define BCM_KNET_FILTER_F_STRIP_INNER_TAG 0x00000004 /* Strip Inner VLAN tag from packets
                                                        sent to virtual network
                                                        inerfaces */
#define BCM_KNET_FILTER_F_PROCESS_VLAN_REMAP 0x00000008 /* Conditionally translate vlan
                                                          as per global remap table*/
#define BCM_KNET_FILTER_F_PROCESS_DOUBLE_TAG 0x00000010 /* Conditionally double tag */
