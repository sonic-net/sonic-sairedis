 ...
 *
 * File:        helix5_flexport.h
 * Purpose:
 * Requires:
 */
 
#ifndef SOC_HELIX5_FLEXPORT_H
#define SOC_HELIX5_FLEXPORT_H
...
#include <soc/flexport/helix5/helix5_idb_flexport.h>
#include <soc/flexport/helix5/helix5_ep_flexport.h>
#include <soc/flexport/helix5/helix5_mac_flexport.h>
...
/*! @struct soc_helix5_flex_scratch_t
...
*/
typedef struct soc_helix5_flex_scratch_s {
    ...
    int prev_hpipe_ovs_ratio[SOC_MAX_NUM_PIPES][HELIX5_OVS_HPIPE_COUNT_PER_PIPE];
    int new_pipe_ovs_ratio[SOC_MAX_NUM_PIPES]; /* per PIPE ovs_ratio
                                                * per Half PIPE ovs_ratio */
    int new_hpipe_ovs_ratio[SOC_MAX_NUM_PIPES][HELIX5_OVS_HPIPE_COUNT_PER_PIPE];
...
} soc_helix5_flex_scratch_t;


/*! @fn void *soc_helix5_port_lane_info_alloc(void)
...
*/
extern void *soc_helix5_port_lane_info_alloc (
  void
  );


/*! @fn int soc_helix5_set_port_lane_info(int unit, const void *lane_info_ptr, int port_idx, int pgw, int xlp)
...
*/
extern int soc_helix5_set_port_lane_info (
  int unit,
...
/*! @fn int soc_helix5_get_port_lane_info(int unit, const void *lane_info_ptr, int *port_idx, int *pgw, int *xlp)
    @param unit Chip unit number.
    @param lane_info_ptr Pointer to a soc_port_lane_info_t struct variable. 
    @param port_idx Index within XLP.
    @param pgw PGW instance where lane resides.
...
*/
extern int soc_helix5_get_port_lane_info (
  int unit,
  const void *lane_info_ptr,
  int *port_idx, 
  int *pgw,
...
/*! @fn int soc_helix5_reconfigure_ports(int unit, soc_port_schedule_state_t *port_schedule_state)
    @param unit Chip unit number.
...
*/
extern int soc_helix5_reconfigure_ports (
  int unit,
  soc_port_schedule_state_t *port_schedule_state
  );


/*! @fn int soc_helix5_flex_start(int unit, soc_port_schedule_state_t *port_schedule_state)
    @param unit Chip unit number.
    @param port_schedule_state Pointer to data holding all required information for FlexPort opeations. 
    @brief The start of FlexPort series of operations along with allocations of required program memory.
*/
extern int soc_helix5_flex_start (
  int unit,
  soc_port_schedule_state_t *port_schedule_state
  );


/*! @fn int soc_helix5_flex_end(int unit, soc_port_schedule_state_t *port_schedule_state)
    @param unit Chip unit number.
    @param port_schedule_state Pointer to data holding all required information for FlexPort opeations. 
    @brief The end of FlexPort series of operations along with deallocations of program memory.
*/
extern int soc_helix5_flex_end (
  int unit,
  soc_port_schedule_state_t *port_schedule_state
  );


extern int soc_helix5_flex_top_port_down (
    int                        unit,
    soc_port_schedule_state_t *port_schedule_state
    );


extern int soc_helix5_flex_top_port_up (
    int                        unit,
    soc_port_schedule_state_t *port_schedule_state
    );

extern int soc_helix5_flex_mmu_port_up_top  (
    int                        unit,
    soc_port_schedule_state_t *port_schedule_state
    );

extern int soc_helix5_get_ct_class(int speed);
...
*/
extern int (*hx5_flexport_phases[MAX_FLEX_PHASES]) (int unit, soc_port_schedule_state_t *port_schedule_state);


/*** END SDK API COMMON CODE ***/

#endif /* SOC_HELIX5_FLEXPORT_H */
