...
#define KCOM_M_VT_CREATE        61 /* Create a vlan translation entry */
#define KCOM_M_VT_DESTROY       62 /* Destroy a vlan translation entry */
#define KCOM_M_VT_LIST          63 /* Get list of vlan translation entries */
#define KCOM_M_VT_GET           64 /* Get vlan translation info */
#define KCOM_M_VXLAN            71 /* VXLAN handling */
#define KCOM_M_INT_VLAN         72 /* Add/Remove VLAN ID to L3 device */
#define KCOM_M_VT_CREATE_INGRESS 73 /* Create ingress vlan translation entry */
#define KCOM_M_VT_CREATE_EGRESS  74 /* Create egress vlan translation entry */
#define KCOM_M_VT_DESTROY_INGRESS 75 /* Destroy ingress vlan translation entry */
#define KCOM_M_VT_DESTROY_EGRESS  76 /* Destroy egress vlan translation entry */
#define KCOM_M_VT_EGRESS_LIST     77 /* Get list of vlan translation egress entries */
#define KCOM_M_VT_EGRESS_GET      78 /* Get vlan translation egress info */
 ...
 *
 *  KCOM_NETIF_F_NO_TX
 *  Do not allow TX on this device.
 *
 */
...
#define KCOM_NETIF_F_TX_HW_ROUTE (1U << 3)
#define KCOM_NETIF_F_TX_VXLAN_ENCAP (1U << 4)
#define KCOM_NETIF_F_NO_TX      (1U << 5)
 ...
 *
 *  KCOM_DEST_T_DYN_NETIF
 *  Packet is sent to port network interface matching ingress port.
 *
 * Filter flags:
 *
...
#define KCOM_DEST_T_DYN_NETIF   4

#define KCOM_FILTER_F_ANY_DATA  (1U << 0)
#define KCOM_FILTER_F_STRIP_TAG (1U << 1)
#define KCOM_FILTER_F_CON_STRIP_TAG (1U << 2)
#define KCOM_FILTER_F_STRIP_INNER_TAG (1U << 3)
#define KCOM_FILTER_F_PROCESS_VLAN_REMAP (1U << 4)
#define KCOM_FILTER_F_PROCESS_DOUBLE_TAG (1U << 5)
 ...
 *
 * VT entry will have a port and vlan as an lookup key and
 * if an entry found, the packet dot1q tag vlan will be
 * relaced with the trans_vlan number and sent to knet
 * virtual interface
 *
 * vt types:
 *
...
/*
 * The maximum OOB size is 16 words , which is 64 bytes.
 * To provide room for future expansion 64+32<8 words> are
 * add to the current size
 */
#define KCOM_VT_BYTES_MAX   96
#define KCOM_VT_WORDS_MAX   BYTES2WORDS(KCOM_VT_BYTES_MAX)

#define KCOM_VT_T_RX_PKT    1

typedef struct kcom_vt_s {
    uint32 id;
    uint8  type;
    uint16 vlan;
    uint16 trans_vlan;
    uint8  src_port;
    uint32 ingress_stats;
    uint32 egress_stats;
} kcom_vt_t;
 ...
 * VXLAN information
 *
 * KCOM_VXLAN_ACTION_ADD
 * Add VNI->VLAN map to knet
 *
 * KCOM_VXLAN_ACTION_DELETE
 * Delete VNI->VLAN map from knet
 *
 * KCOM_VXLAN_ACTION_UPDATE
 * Update VNI->VLAN map in knet
 *
 * KCOM_VXLAN_ACTION_GET
 * GET VNI->VLAN map in knet
 *
 * KCOM_VXLAN_ACTION_DEBUG
 * Enable/disable debug in knet
 */
#define KCOM_VXLAN_OK            0
#define KCOM_VXLAN_FAIL          -1

#define KCOM_VXLAN_ACTION_ADD    1
#define KCOM_VXLAN_ACTION_DELETE 2
#define KCOM_VXLAN_ACTION_UPDATE 3
#define KCOM_VXLAN_ACTION_GET    4
#define KCOM_VXLAN_ACTION_DEBUG  100

typedef struct kcom_vxlan_s {
    uint8      action;
    uint32     vni;
    uint16     vlan;
    uint8      is_l3vni;
    uint8      debug_flag;
    uint8 outer_encap_mac[ETH_ALEN]; /* required for overlay tx*/
  } kcom_vxlan_t;

typedef struct kcom_int_vlan_s {
    int knet_if_id;
    int vlan;
} kcom_int_vlan_t;
...
/*
 * Create new vlan translation entry
 * The vt id will be assigned by the kernel module.
 */
typedef struct kcom_msg_vt_create_s {
    kcom_msg_hdr_t hdr;
    kcom_vt_t vt;
} kcom_msg_vt_create_t;

/*
 * Destroy vlan translation entry
 */
typedef struct kcom_msg_vt_destroy_s {
    kcom_msg_hdr_t hdr;
    uint16 port;
    uint16 vlan;
} kcom_msg_vt_destroy_t;

/*
 * Get list of currently defined vt entries
 */
#ifndef KCOM_VT_MAX
#define KCOM_VT_MAX         128
#endif

typedef struct kcom_vt_list_s {
    uint32 hw_port;
    uint32 fcnt;
    uint32 ccnt;
    uint32 id[KCOM_VT_MAX];
} kcom_vt_list_t;

typedef struct kcom_msg_vt_list_s {
    kcom_msg_hdr_t hdr;
    kcom_vt_list_t list;
} kcom_msg_vt_list_t;

/*
 * Get detailed vt information.
 */
typedef struct kcom_msg_vt_get_s {
    kcom_msg_hdr_t hdr;
    kcom_vt_t vt;
} kcom_msg_vt_get_t;
 ...
 * VXLAN info
 */
typedef struct kcom_msg_vxlan_s {
    kcom_msg_hdr_t hdr;
    kcom_vxlan_t vxlan;
} kcom_msg_vxlan_t;
 ...
 * Internal Vlan Info
 */
typedef struct kcom_msg_int_vlan_s{
    kcom_msg_hdr_t hdr;
    kcom_int_vlan_t vlan_info;
} kcom_msg_int_vlan_t;
...
typedef union kcom_msg_s {
    ...
    kcom_msg_vt_create_t vt_create;
    kcom_msg_vt_destroy_t vt_destroy;
    kcom_msg_vt_list_t vt_list;
    kcom_msg_vt_get_t vt_get;
    kcom_msg_vxlan_t vxlan;
    kcom_msg_int_vlan_t internal_vlan;
} kcom_msg_t;
