...
std::shared_ptr<sairedis::SaiInterface> proxy_sai = std::make_shared<Sai>();
        ...
        _In_ const sai_service_method_table_t* service_method_table)
{
    ...
    return proxy_sai->initialize(flags, service_method_table);
}
...
#define API(api) .api ## _api = const_cast<sai_ ## api ## _api_t*>(&proxy_ ## api ## _api)

static sai_apis_t proxy_apis = {
    API(switch),
    API(port),
    API(fdb),
    API(vlan),
        ...
        _Out_ void** api_method_table)
{
    ...
    {
        *api_method_table = ((void**)&proxy_apis)[sai_api_id - 1];
        ...
        _Out_ sai_attr_capability_t *capability)
{
    ...
    return proxy_sai->queryAttributeCapability(
            switch_id,
            object_type,
            attr_id,
            capability);
        ...
        _Inout_ sai_s32_list_t *enum_values_capability)
{
    ...
    return proxy_sai->queryAattributeEnumValuesCapability(
            switch_id,
            object_type,
            attr_id,
        ...
        _Out_ uint64_t *count)
{
    ...
    return proxy_sai->objectTypeGetAvailability(
            switch_id,
            object_type,
            attr_count,
            attr_list,
        ...
        _In_ sai_object_id_t objectId)
{
    ...
    return proxy_sai->objectTypeQuery(objectId);
}
        ...
        _In_ sai_object_id_t objectId)
{
    ...
    return proxy_sai->switchIdQuery(objectId);
}
