...
/*************************************************************
* Function    : sai_deserialize_stats_capability_list
* Parameters  :
*              stats_capability - stats capability enum list
*              stat_enum_str    - SAI stat enum list as string
*              stat_modes_str   - SAI stat mode list as string
* Description :
*              1. Iterate thru stat_enum_str and populate
*                 the stats_capability with respective
*                 stat_enum (String to Enum conversion)
*              2. Iterate thru stat_modes_str and populate
*                 the stats_capability with respective
        ...
        _In_    const std::string& stat_modes_str)
{
    ...
    for (uint32_t i = 0; i < num_capabilities; i++)
    {
        ...
        if (stat_enum_position == std::string::npos)
        {
            ...
            {
                SWSS_LOG_WARN("Lesser stat_enums than expected: expected %d, received %d",
        ...
        if (stat_modes_position == std::string::npos)
        {
            if (num_capabilities != i + 1)
            {
                SWSS_LOG_WARN("Lesser stat_modes than expected: expected %d, received %d",
                        num_capabilities, i+1);
            }
