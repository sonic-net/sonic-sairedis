...
otai_status_t DummyOtaiInterface::apiInitialize(
        _In_ uint64_t flags,
        _In_ const otai_service_method_table_t *smt)
{
    SWSS_LOG_ENTER();
...
otai_status_t DummyOtaiInterface::apiUninitialize(void)
{
    SWSS_LOG_ENTER();
        ...
        _In_ const otai_attribute_t *attr_list)
{
    ...
    if (objectId && m_status == OTAI_STATUS_SUCCESS)
    {
        *objectId = (otai_object_id_t)1;
    }

    if (m_status == OTAI_STATUS_SUCCESS && objectType == OTAI_OBJECT_TYPE_LINECARD)
    {
...
otai_status_t DummyOtaiInterface::bulkCreate(             \
...
otai_status_t DummyOtaiInterface::bulkRemove(             \
        _In_ uint32_t object_count,                     \
        _In_ const otai_ ## ot ## _t* ot,                \
        _In_ otai_bulk_op_error_mode_t mode,             \
        _Out_ otai_status_t *object_statuses)            \
...
otai_status_t DummyOtaiInterface::bulkSet(                \
        _In_ uint32_t object_count,                     \
        _In_ const otai_ ## ot ## _t* ot,                \
        _In_ const otai_attribute_t *attr_list,          \
        _In_ otai_bulk_op_error_mode_t mode,             \
...
otai_status_t DummyOtaiInterface::bulkGet(                    \
        _In_ uint32_t object_count,                         \
        _In_ const otai_ ## ot ## _t *ot,                    \
        _In_ const uint32_t *attr_count,                    \
        _Inout_ otai_attribute_t **attr_list,                \
...
otai_status_t DummyOtaiInterface::flushFdbEntries(
        _In_ otai_object_id_t switchId,
...
otai_status_t DummyOtaiInterface::switchMdioRead(
    _In_ otai_object_id_t switchId,
...
otai_status_t DummyOtaiInterface::switchMdioWrite(
    _In_ otai_object_id_t switchId,
    _In_ uint32_t device_addr,
    _In_ uint32_t start_reg_addr,
    _In_ uint32_t number_of_registers,
...
otai_status_t DummyOtaiInterface::switchMdioCl22Read(
    _In_ otai_object_id_t switchId,
    _In_ uint32_t device_addr,
    _In_ uint32_t start_reg_addr,
    _In_ uint32_t number_of_registers,
...
otai_status_t DummyOtaiInterface::switchMdioCl22Write(
    _In_ otai_object_id_t switchId,
    _In_ uint32_t device_addr,
    _In_ uint32_t start_reg_addr,
    _In_ uint32_t number_of_registers,
...
otai_status_t DummyOtaiInterface::queryAttributeEnumValuesCapability(
        _In_ otai_object_id_t switchId,
        _In_ otai_object_type_t objectType,
...
otai_status_t DummyOtaiInterface::getStats(
...
otai_status_t DummyOtaiInterface::getStatsExt(
        _In_ otai_object_type_t object_type,
        _In_ otai_object_id_t object_id,
        _In_ uint32_t number_of_counters,
        _In_ const otai_stat_id_t *counter_ids,
...
otai_status_t DummyOtaiInterface::clearStats(
        _In_ otai_object_type_t object_type,
        _In_ otai_object_id_t object_id,
        _In_ uint32_t number_of_counters,
        _In_ const otai_stat_id_t *counter_ids)
...
otai_status_t DummyOtaiInterface::bulkGetStats(
        _In_ otai_object_id_t switchId,
        _In_ otai_object_type_t object_type,
        _In_ uint32_t object_count,
...
otai_status_t DummyOtaiInterface::bulkClearStats(
        _In_ otai_object_id_t switchId,
        _In_ otai_object_type_t object_type,
        _In_ uint32_t object_count,
        _In_ const otai_object_key_t *object_key,
...
otai_object_id_t DummyOtaiInterface::switchIdQuery(
        _In_ otai_object_id_t objectId)
{
    SWSS_LOG_ENTER();
...
//otai_status_t DummyOtaiInterface::queryApiVersion(
...
void DummyOtaiInterface::updateNotificationPointers(
        _In_ uint32_t count,
        _In_ const otai_attribute_t* attrs)
{
    ...
    throw;
    // otai_metadata_update_switch_notification_pointers(&m_sn, count, attrs); // not exist
}

otai_status_t DummyOtaiInterface::start()
{
    SWSS_LOG_ENTER();
    ...
    m_thread = std::make_shared<std::thread>(&DummyOtaiInterface::run, this);

    return OTAI_STATUS_SUCCESS;
}

otai_status_t DummyOtaiInterface::stop()
{
    SWSS_LOG_ENTER();
...
otai_status_t DummyOtaiInterface::enqueueNotificationToSend(
...
bool DummyOtaiInterface::tryGetNotificationToSend(
...
void DummyOtaiInterface::sendNotification(
        _In_ otai_attr_id_t id)
{
    SWSS_LOG_ENTER();
    ...
    //otai_linecard_notifications_t sn = m_sn;

    auto* m = otai_metadata_get_attr_metadata(OTAI_OBJECT_TYPE_LINECARD, id);

    switch (id)
    {
        //case OTAI_LINECARD_ATTR_SWITCH_STATE_CHANGE_NOTIFY:

        //    if (sn.on_switch_state_change)
        //    {
        //        SWSS_LOG_NOTICE("sending sn.on_switch_state_change");

        //        sn.on_switch_state_change(0x1, OTAI_LINECARD_OPER_STATUS_UP);
        //    }
        //    else
        //    {
        ...
        //case OTAI_LINECARD_ATTR_FDB_EVENT_NOTIFY:
        ...
        //        data.event_type = OTAI_FDB_EVENT_LEARNED;
        ...
        //case OTAI_LINECARD_ATTR_PORT_STATE_CHANGE_NOTIFY:

        //    if (sn.on_port_state_change)
        //    {
        //        SWSS_LOG_NOTICE("sending sn.on_port_state_change");

        //        otai_port_oper_status_notification_t data;

        //        data.port_id = 0x2;
        //        data.port_state = OTAI_PORT_OPER_STATUS_UP;
        ...
        //case OTAI_LINECARD_ATTR_SHUTDOWN_REQUEST_NOTIFY:
        ...
        //case OTAI_LINECARD_ATTR_NAT_EVENT_NOTIFY:

        //    if (sn.on_nat_event)
        //    {
        //        SWSS_LOG_NOTICE("sending sn.on_nat_event");

        //        otai_nat_event_notification_data_t data;

        //        data.event_type = OTAI_NAT_EVENT_NONE;
        //        data.nat_entry.switch_id = 0x1;
        //        data.nat_entry.vr_id = 0x2;
        //        data.nat_entry.nat_type = OTAI_NAT_TYPE_NONE;
        ...
        //case OTAI_LINECARD_ATTR_PORT_HOST_TX_READY_NOTIFY:

        //    if (sn.on_nat_event)
        //    {
        //        SWSS_LOG_NOTICE("sending sn.on_port_host_tx_ready");

        //        sn.on_port_host_tx_ready(0x1, 0x2, OTAI_PORT_HOST_TX_READY_STATUS_NOT_READY);
        //    }
        //    else
        //    {
        ...
        //case OTAI_LINECARD_ATTR_SWITCH_ASIC_SDK_HEALTH_EVENT_NOTIFY:

        //    if (sn.on_nat_event)
        //    {
        //        SWSS_LOG_NOTICE("sending sn.on_switch_asic_sdk_health_event");

        //        otai_timespec_t timespec;

        //        timespec.tv_sec = 0;
        //        timespec.tv_nsec = 0;

        //        otai_switch_health_data_t hd;

        //        hd.data_type = OTAI_HEALTH_DATA_TYPE_GENERAL;
        ...
        //                0x1,
        //                OTAI_LINECARD_ASIC_SDK_HEALTH_SEVERITY_NOTICE,
        //                timespec,
        //                OTAI_LINECARD_ASIC_SDK_HEALTH_CATEGORY_SW,
        //                hd,
        //                desc);
        //    }
        //    else
        ...
        //case OTAI_LINECARD_ATTR_QUEUE_PFC_DEADLOCK_NOTIFY:

        //    if (sn.on_queue_pfc_deadlock)
        //    {
        //        SWSS_LOG_NOTICE("sending sn.on_queue_pfc_deadlock");

        //        otai_queue_deadlock_notification_data_t data;

        //        data.queue_id = 0x2;
        //        data.event = OTAI_QUEUE_PFC_DEADLOCK_EVENT_TYPE_DETECTED;
        ...
        //case OTAI_LINECARD_ATTR_BFD_SESSION_STATE_CHANGE_NOTIFY:

        //    if (sn.on_bfd_session_state_change)
        //    {
        //        SWSS_LOG_NOTICE("sending sn.on_bfd_session_state_change");

        //        otai_bfd_session_state_notification_t data;

        //        data.bfd_session_id = 0x2;
        //        data.session_state = OTAI_BFD_SESSION_STATE_ADMIN_DOWN;
        ...
        //case OTAI_LINECARD_ATTR_TWAMP_SESSION_EVENT_NOTIFY:

        //    if (sn.on_twamp_session_event)
        //    {
        //        SWSS_LOG_NOTICE("sending sn.on_twamp_session_event");

        //        otai_twamp_session_event_notification_data_t data;

        //        data.twamp_session_id = 0x1;
        //        data.session_state = OTAI_TWAMP_SESSION_STATE_INACTIVE;
        ...
        default:
            ...
            SWSS_LOG_WARN("notification for LINECARD attr id: %d (%s) is not supported, FIXME", id, (m ? m->attridname : "UNKNOWN"));
            break;
    }
}
