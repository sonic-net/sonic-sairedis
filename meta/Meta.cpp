...
{                                                                               \
    ...
    if (_status != OTAI_STATUS_SUCCESS)                                          \
    {                                                                           \
        return _status;                                                         \
    }                                                                           \
}
...
#define META_LOG_STATUS(status,msg)                                                     \
    ...
    { SWSS_LOG_DEBUG(msg " status: %s", otai_serialize_status(status).c_str()); }        \
    else { SWSS_LOG_ERROR(msg " status: %s", otai_serialize_status(status).c_str()); }
...
{                                                                     \
    SWSS_LOG_ENTER();                                                 \
    otai_status_t status = meta_otai_validate_ ## ot (ot, true);        \
    CHECK_STATUS_SUCCESS(status);                                     \
    otai_object_meta_key_t meta_key = {                                \
         .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_ ## OT,     \
         .objectkey = { .key = { .ot = *ot } } };                     \
...
{                                                                \
    SWSS_LOG_ENTER();                                            \
    otai_status_t status = meta_otai_validate_ ## ot (ot, false);  \
    CHECK_STATUS_SUCCESS(status);                                \
    otai_object_meta_key_t meta_key = {                           \
        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_ ## OT, \
        .objectkey = { .key = { .ot = *ot  } }                   \
...
{                                                                              \
    SWSS_LOG_ENTER();                                                          \
    otai_status_t status = meta_otai_validate_ ## ot (ot, false, true);          \
    CHECK_STATUS_SUCCESS(status);                                              \
    otai_object_meta_key_t meta_key = {                                         \
        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_ ## OT,               \
        .objectkey = { .key = { .ot = *ot } } };                               \
...
otai_status_t Meta::apiInitialize(
        _In_ uint64_t flags,
...
otai_status_t Meta::apiUninitialize(void)
{
    SWSS_LOG_ENTER();
...
void Meta::dump() const
{
    ...
    SWSS_LOG_NOTICE("otaiObjectCollection: %zu", m_otaiObjectCollection.getAllKeys().size());

    for (auto &oid: m_oids.getAllReferences())
    {
        SWSS_LOG_NOTICE("oid: %s: count: %u",
                otai_serialize_object_id(oid.first).c_str(),
                oid.second);
    }

    for (auto &mk: m_otaiObjectCollection.getAllKeys())
    {
        SWSS_LOG_NOTICE("objcollection: %s", otai_serialize_object_meta_key(mk).c_str());
    }
}
        ...
        _In_ const otai_attribute_t *attr_list)
{
    ...
    otai_status_t status = meta_otai_validate_oid(object_type, object_id, switch_id, true);
        ...
        _In_ const otai_attribute_t *attr)
{
    ...
    otai_object_id_t switch_id = switchIdQuery(object_id);
...
//otai_status_t Meta::flushFdbEntries(
//        _In_ otai_object_id_t switch_id,
//        _In_ uint32_t attr_count,
//        _In_ const otai_attribute_t *attr_list)
//{
...
//    }
//
//    otai_object_type_t swot = objectTypeQuery(switch_id);
//
//    if (swot != OTAI_OBJECT_TYPE_LINECARD)
//    {
//        SWSS_LOG_ERROR("object type for switch_id %s is %s",
//                otai_serialize_object_id(switch_id).c_str(),
//                otai_serialize_object_type(swot).c_str());
//
//        return OTAI_STATUS_INVALID_PARAMETER;
//    }
//
...
//
//        auto mdp = otai_metadata_get_attr_metadata(OTAI_OBJECT_TYPE_FDB_FLUSH, attr->id);
//
//        if (mdp == NULL)
//        {
//            SWSS_LOG_ERROR("unable to find attribute metadata OTAI_OBJECT_TYPE_FDB_FLUSH:%d", attr->id);
//
//            return OTAI_STATUS_INVALID_PARAMETER;
//        }
//
...
//
//                if (md.isenum && !otai_metadata_is_allowed_enum_value(&md, value.s32))
//                {
...
//
//                {
//                    otai_status_t status = meta_generic_validation_objlist(md, switch_id, 1, &value.oid);
//
//                    CHECK_STATUS_SUCCESS(status)
//
//                    break;
...
//                        otai_serialize_attr_value_type(md.attrvaluetype).c_str());
//        }
//    }
//
...
//
//        auto *et = otai_metadata_get_attr_by_id(OTAI_FDB_FLUSH_ATTR_ENTRY_TYPE, attr_count, attr_list);
//
//        if (et)
//        {
//            switch (et->value.s32)
//            {
//                case OTAI_FDB_FLUSH_ENTRY_TYPE_DYNAMIC:
//                    types.push_back(OTAI_FDB_ENTRY_TYPE_DYNAMIC);
//                    break;
//
//                case OTAI_FDB_FLUSH_ENTRY_TYPE_STATIC:
//                    types.push_back(OTAI_FDB_ENTRY_TYPE_STATIC);
//                    break;
//
//                default:
//                    types.push_back(OTAI_FDB_ENTRY_TYPE_DYNAMIC);
...
//        for (auto type: types)
//        {
//            otai_fdb_event_notification_data_t data = {};
//
//            auto *bv_id = otai_metadata_get_attr_by_id(OTAI_FDB_FLUSH_ATTR_BV_ID, attr_count, attr_list);
//            auto *bp_id = otai_metadata_get_attr_by_id(OTAI_FDB_FLUSH_ATTR_BRIDGE_PORT_ID, attr_count, attr_list);
//
//            otai_attribute_t list[2];
//
//            list[0].id = OTAI_FDB_ENTRY_ATTR_BRIDGE_PORT_ID;
//            list[0].value.oid = bp_id ? bp_id->value.oid : OTAI_NULL_OBJECT_ID;
//
//            list[1].id = OTAI_FDB_ENTRY_ATTR_TYPE;
//            list[1].value.s32 = type;
//
//            data.event_type = OTAI_FDB_EVENT_FLUSHED;
//            data.fdb_entry.switch_id = switch_id;
//            data.fdb_entry.bv_id = (bv_id) ? bv_id->value.oid : OTAI_NULL_OBJECT_ID;
//            data.attr_count = 2;
//            data.attr = list;
//
//            meta_otai_on_fdb_flush_event_consolidated(data);
//        }
//    }
//
//    return status;
...
{                                                                                                                       \
    ...
    {                                                                                                                   \
        object_statuses[idx] = OTAI_STATUS_NOT_EXECUTED;                                                                 \
    }                                                                                                                   \
    ...
    if (otai_metadata_get_enum_value_name(&otai_metadata_enum_otai_bulk_op_error_mode_t, mode) == nullptr)                 \
    {                                                                                                                   \
        SWSS_LOG_ERROR("mode value %d is not in range on %s", mode, otai_metadata_enum_otai_bulk_op_error_mode_t.name);   \
        return OTAI_STATUS_INVALID_PARAMETER;                                                                            \
    }                                                                                                                   \
    std::vector<otai_object_meta_key_t> vmk;                                                                             \
    for (uint32_t idx = 0; idx < object_count; idx++)                                                                   \
    {                                                                                                                   \
        otai_status_t status = meta_otai_validate_ ##ot (&ot[idx], true);                                                 \
        CHECK_STATUS_SUCCESS(status);                                                                                   \
        otai_object_meta_key_t meta_key = {                                                                              \
            .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_ ## OT,                                                    \
...
{                                                                                                                       \
    ...
    for (uint32_t idx = 0; idx < object_count; idx++)                                                                   \
    {                                                                                                                   \
        otai_status_t status = meta_otai_validate_ ##ot (&ot[idx], false);                                                \
        CHECK_STATUS_SUCCESS(status);                                                                                   \
        otai_object_meta_key_t meta_key = {                                                                              \
            .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_ ## OT,                                                    \
            .objectkey = { .key = { .ot = ot[idx] } }                                                                   \
        ...
        _Out_ uint64_t *count)
{
    ...
    PARAMETER_CHECK_OID_OBJECT_TYPE(switchId, OTAI_OBJECT_TYPE_LINECARD);
    PARAMETER_CHECK_OID_EXISTS(switchId, OTAI_OBJECT_TYPE_LINECARD);
    ...
    for (uint32_t idx = 0; idx < attrCount; idx++)
    {
        ...
        switch (mdp->attrvaluetype)
        {
            ...
            case OTAI_ATTR_VALUE_TYPE_OBJECT_ID:
            {
                otai_object_type_t ot = objectTypeQuery(attrList[idx].value.oid);
...
otai_status_t Meta::queryAttributeEnumValuesCapability(
        _In_ otai_object_id_t switchId,
        _In_ otai_object_type_t objectType,
        _In_ otai_attr_id_t attrId,
        ...
        _In_ otai_stats_mode_t mode)
{
    ...
    otai_status_t status = meta_otai_validate_oid(object_type, &object_id, switch_id, false);
...
otai_status_t Meta::meta_validate_query_stats_capability(
        _In_ otai_object_type_t object_type,
        _In_ otai_object_id_t object_id)
{
    SWSS_LOG_ENTER();
...
//otai_status_t Meta::queryStatsCapability(
//        _In_ otai_object_id_t switchId,
//        _In_ otai_object_type_t objectType,
//        _Inout_ otai_stat_capability_list_t *stats_capability)
//{
//    SWSS_LOG_ENTER();
//
...
otai_status_t Meta::bulkGetStats(
        _In_ otai_object_id_t switchId,
        _In_ otai_object_type_t object_type,
        _In_ uint32_t object_count,
...
otai_status_t Meta::bulkClearStats(
        _In_ otai_object_id_t switchId,
        _In_ otai_object_type_t object_type,
        _In_ uint32_t object_count,
        _In_ const otai_object_key_t *object_key,
        ...
        _Out_ otai_status_t *object_statuses)
{
    ...
    for (uint32_t idx = 0; idx < object_count; idx++)
    {
        otai_status_t status = meta_otai_validate_oid(object_type, &object_id[idx], OTAI_NULL_OBJECT_ID, false);

        CHECK_STATUS_SUCCESS(status);

        otai_object_meta_key_t meta_key = { .objecttype = object_type, .objectkey = { .key = { .object_id  = object_id[idx] } } };
        ...
        _Out_ otai_status_t *object_statuses)
{
    ...
    for (uint32_t idx = 0; idx < object_count; idx++)
    {
        otai_status_t status = meta_otai_validate_oid(object_type, &object_id[idx], switchId, true);
...
otai_object_id_t Meta::switchIdQuery(
        _In_ otai_object_id_t objectId)
{
    SWSS_LOG_ENTER();
        ...
        _In_ otai_log_level_t log_level)
{
    ...
    if (otai_metadata_get_enum_value_name(&otai_metadata_enum_otai_api_t, api) == nullptr)
    {
        SWSS_LOG_ERROR("api value %d is not in range on %s", api, otai_metadata_enum_otai_api_t.name);

        return OTAI_STATUS_INVALID_PARAMETER;
    }

    if (otai_metadata_get_enum_value_name(&otai_metadata_enum_otai_log_level_t, log_level) == nullptr)
    {
        SWSS_LOG_ERROR("log level value %d is not in range on %s", log_level, otai_metadata_enum_otai_log_level_t.name);
...
//otai_status_t Meta::queryApiVersion(
//        _Out_ otai_api_version_t *version)
//{
//    SWSS_LOG_ENTER();
//
        ...
        _In_ otai_object_id_t switchId)
{
    ...
    if (objectTypeQuery(switchId) != OTAI_OBJECT_TYPE_LINECARD)
    {
        ...
        _In_ const otai_object_meta_key_t& meta_key)
{
    ...
    if (meta_key.objecttype == OTAI_OBJECT_TYPE_PORT)
    {
...
//otai_status_t Meta::meta_otai_validate_fdb_entry(
...
//
//    otai_object_meta_key_t meta_key_fdb = { .objecttype = OTAI_OBJECT_TYPE_FDB_ENTRY, .objectkey = { .key = { .fdb_entry = *fdb_entry } } };
//
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_fdb))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_fdb).c_str());
//
...
//    // set, get, remove
//
//    if (!m_otaiObjectCollection.objectExists(meta_key_fdb) && !get)
//    {
//        SWSS_LOG_ERROR("object key %s doesn't exist",
//                otai_serialize_object_meta_key(meta_key_fdb).c_str());
//
...
//otai_status_t Meta::meta_otai_validate_mcast_fdb_entry(
...
//    }
//
//    otai_object_id_t bv_id = mcast_fdb_entry->bv_id;
//
//    if (bv_id == OTAI_NULL_OBJECT_ID)
//    {
...
//    }
//
//    otai_object_type_t object_type = objectTypeQuery(bv_id);
//
...
//    }
//
//    if (object_type != OTAI_OBJECT_TYPE_BRIDGE && object_type != OTAI_OBJECT_TYPE_VLAN)
//    {
...
//
//    otai_object_meta_key_t meta_key_bv = { .objecttype = object_type, .objectkey = { .key = { .object_id = bv_id } } };
//
//    if (!m_otaiObjectCollection.objectExists(meta_key_bv))
//    {
//        SWSS_LOG_ERROR("object key %s doesn't exist",
//                otai_serialize_object_meta_key(meta_key_bv).c_str());
//
//        return OTAI_STATUS_ITEM_NOT_FOUND;
//    }
//
//    // check if fdb entry exists
//
//    otai_object_meta_key_t meta_key_fdb = { .objecttype = OTAI_OBJECT_TYPE_MCAST_FDB_ENTRY, .objectkey = { .key = { .mcast_fdb_entry = *mcast_fdb_entry } } };
//
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_fdb))
...
//otai_status_t Meta::meta_otai_validate_neighbor_entry(
...
//    }
//
//    otai_object_id_t rif = neighbor_entry->rif_id;
//
//    if (rif == OTAI_NULL_OBJECT_ID)
//    {
...
//    }
//
//    otai_object_type_t object_type = objectTypeQuery(rif);
//
//    if (object_type == OTAI_OBJECT_TYPE_NULL)
//    {
...
//    }
//
//    otai_object_type_t expected = OTAI_OBJECT_TYPE_ROUTER_INTERFACE;
//
...
//
//    otai_object_meta_key_t meta_key_rif = { .objecttype = expected, .objectkey = { .key = { .object_id = rif } } };
//
//    if (!m_otaiObjectCollection.objectExists(meta_key_rif))
//    {
//        SWSS_LOG_ERROR("object key %s doesn't exist",
//                otai_serialize_object_meta_key(meta_key_rif).c_str());
//
//        return OTAI_STATUS_ITEM_NOT_FOUND;
//    }
//
//    otai_object_meta_key_t meta_key_neighbor = { .objecttype = OTAI_OBJECT_TYPE_NEIGHBOR_ENTRY, .objectkey = { .key = { .neighbor_entry = *neighbor_entry } } };
//
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_neighbor))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_neighbor).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//otai_status_t Meta::meta_otai_validate_route_entry(
...
//            {
//                SWSS_LOG_ERROR("invalid ipv6 mask: %s", otai_serialize_ipv6(route_entry->destination.mask.ip6).c_str());
//
//                return OTAI_STATUS_INVALID_PARAMETER;
//            }
//
...
//    }
//
//    otai_object_id_t vr = route_entry->vr_id;
//
//    if (vr == OTAI_NULL_OBJECT_ID)
//    {
...
//    }
//
//    otai_object_type_t object_type = objectTypeQuery(vr);
//
//    if (object_type == OTAI_OBJECT_TYPE_NULL)
//    {
...
//    }
//
//    otai_object_type_t expected = OTAI_OBJECT_TYPE_VIRTUAL_ROUTER;
//
//    if (object_type != expected)
//    {
...
//
//    otai_object_meta_key_t meta_key_vr = { .objecttype = expected, .objectkey = { .key = { .object_id = vr } } };
//
//    if (!m_otaiObjectCollection.objectExists(meta_key_vr))
//    {
//        SWSS_LOG_ERROR("object key %s doesn't exist",
//                otai_serialize_object_meta_key(meta_key_vr).c_str());
//
//        return OTAI_STATUS_ITEM_NOT_FOUND;
//    }
//
...
//
//    otai_object_meta_key_t meta_key_route = { .objecttype = OTAI_OBJECT_TYPE_ROUTE_ENTRY, .objectkey = { .key = { .route_entry = *route_entry } } };
//
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_route))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_route).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_l2mc_entry(
...
//    {
//        case OTAI_L2MC_ENTRY_TYPE_SG:
//        case OTAI_L2MC_ENTRY_TYPE_XG:
//            break;
//
//        default:
//
...
//    }
//
//    otai_object_id_t bv_id = l2mc_entry->bv_id;
//
//    if (bv_id == OTAI_NULL_OBJECT_ID)
//    {
//        SWSS_LOG_ERROR("bv_id set to null object id");
...
//
//    otai_object_meta_key_t meta_key_route = { .objecttype = OTAI_OBJECT_TYPE_L2MC_ENTRY, .objectkey = { .key = { .l2mc_entry = *l2mc_entry } } };
//
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_route))
...
//}
//
//otai_status_t Meta::meta_otai_validate_ipmc_entry(
...
//    {
//        case OTAI_IPMC_ENTRY_TYPE_SG:
//        case OTAI_IPMC_ENTRY_TYPE_XG:
//            break;
//
//        default:
//
...
//    }
//
//    otai_object_id_t vr_id = ipmc_entry->vr_id;
//
//    if (vr_id == OTAI_NULL_OBJECT_ID)
//    {
...
//    }
//
//    otai_object_type_t object_type = objectTypeQuery(vr_id);
//
//    if (object_type == OTAI_OBJECT_TYPE_NULL)
//    {
...
//    }
//
//    if (object_type != OTAI_OBJECT_TYPE_VIRTUAL_ROUTER)
//    {
...
//
//    otai_object_meta_key_t meta_key_bv = { .objecttype = object_type, .objectkey = { .key = { .object_id = vr_id } } };
//
//    if (!m_otaiObjectCollection.objectExists(meta_key_bv))
//    {
//        SWSS_LOG_ERROR("object key %s doesn't exist",
...
//
//    otai_object_meta_key_t meta_key_route = { .objecttype = OTAI_OBJECT_TYPE_IPMC_ENTRY, .objectkey = { .key = { .ipmc_entry = *ipmc_entry } } };
//
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_route))
...
//}
//
//otai_status_t Meta::meta_otai_validate_nat_entry(
...
//    }
//
//    otai_object_id_t vr = nat_entry->vr_id;
//
//    if (vr == OTAI_NULL_OBJECT_ID)
//    {
//        SWSS_LOG_ERROR("virtual router is set to null object id");
...
//    otai_object_meta_key_t meta_key_nat = { .objecttype = OTAI_OBJECT_TYPE_NAT_ENTRY, .objectkey = { .key = { .nat_entry = *nat_entry } } };
//
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_nat))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_nat).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_inseg_entry(
...
//}
//
//otai_status_t Meta::meta_otai_validate_my_sid_entry(
...
//    }
//
//    otai_object_id_t vr = my_sid_entry->vr_id;
//
//    if (vr == OTAI_NULL_OBJECT_ID)
//    {
//        SWSS_LOG_ERROR("virtual router is set to null object id");
...
//    otai_object_meta_key_t meta_key_my_sid_entry = { .objecttype = OTAI_OBJECT_TYPE_MY_SID_ENTRY, .objectkey = { .key = { .my_sid_entry = *my_sid_entry } } };
//
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_my_sid_entry))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_my_sid_entry).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_direction_lookup_entry(
...
//    }
//
//    otai_object_meta_key_t meta_key_direction_lookup_entry = {
//        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_DIRECTION_LOOKUP_ENTRY,
//        .objectkey = {
...
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_direction_lookup_entry))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_direction_lookup_entry).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_eni_ether_address_map_entry(
...
//    }
//
//    otai_object_meta_key_t meta_key_eni_ether_address_map_entry = {
//        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_ENI_ETHER_ADDRESS_MAP_ENTRY,
//        .objectkey = {
...
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_eni_ether_address_map_entry))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_eni_ether_address_map_entry).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_vip_entry(
...
//    }
//
//    otai_object_meta_key_t meta_key_vip_entry = {
//        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_VIP_ENTRY,
//        .objectkey = {
...
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_vip_entry))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_vip_entry).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_inbound_routing_entry(
...
//    }
//
//    otai_object_meta_key_t meta_key_inbound_routing_entry = {
//        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_INBOUND_ROUTING_ENTRY,
//        .objectkey = {
...
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_inbound_routing_entry))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_inbound_routing_entry).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_pa_validation_entry(
...
//    }
//
//    otai_object_meta_key_t meta_key_pa_validation_entry = {
//        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_PA_VALIDATION_ENTRY,
//        .objectkey = {
...
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_pa_validation_entry))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_pa_validation_entry).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_outbound_routing_entry(
...
//    }
//
//    otai_object_meta_key_t meta_key_outbound_routing_entry = {
//        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_OUTBOUND_ROUTING_ENTRY,
//        .objectkey = {
...
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_outbound_routing_entry))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_outbound_routing_entry).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_outbound_ca_to_pa_entry(
...
//    }
//
//    otai_object_meta_key_t meta_key_outbound_ca_to_pa_entry = {
//        .objecttype = (otai_object_type_t)OTAI_OBJECT_TYPE_OUTBOUND_CA_TO_PA_ENTRY,
//        .objectkey = {
...
//    if (create)
//    {
//        if (m_otaiObjectCollection.objectExists(meta_key_outbound_ca_to_pa_entry))
//        {
//            SWSS_LOG_ERROR("object key %s already exists",
//                    otai_serialize_object_meta_key(meta_key_outbound_ca_to_pa_entry).c_str());
//
//            return OTAI_STATUS_ITEM_ALREADY_EXISTS;
//        }
//
...
//}
//
//otai_status_t Meta::meta_otai_validate_flow_entry(
...
//otai_status_t Meta::meta_otai_validate_meter_bucket_entry(
        ...
        _In_ const otai_attribute_t *attr_list)
{
    ...
    bool switchcreate = meta_key.objecttype == OTAI_OBJECT_TYPE_LINECARD;

    if (switchcreate)
    {
        // we are creating switch

        switch_id = OTAI_NULL_OBJECT_ID;
    ...
    else
    {
        ...
        if (switch_id == OTAI_NULL_OBJECT_ID)
        {
            SWSS_LOG_ERROR("switch id is NULL for %s", otai_serialize_object_type(meta_key.objecttype).c_str());

            return OTAI_STATUS_INVALID_PARAMETER;
        }

        otai_object_type_t sw_type = objectTypeQuery(switch_id);

        if (sw_type != OTAI_OBJECT_TYPE_LINECARD)
        {
            SWSS_LOG_ERROR("switch id 0x%" PRIx64 " type is %s, expected SWITCH", switch_id, otai_serialize_object_type(sw_type).c_str());
        ...
        otai_object_meta_key_t switch_meta_key = { .objecttype = OTAI_OBJECT_TYPE_LINECARD, .objectkey = { .key = { .object_id = switch_id } } };

        if (!m_otaiObjectCollection.objectExists(switch_meta_key))
        {
    ...
    otai_status_t status = meta_generic_validate_non_object_on_create(meta_key, switch_id);
...
#define OTAI_HOSTIF_NAME_SIZE 31

                    size_t len = strnlen(chardata, OTAI_HOSTIF_NAME_SIZE);

                    if (len == OTAI_HOSTIF_NAME_SIZE)
                    {
    ...
    for (auto mdp: metadata)
    {
        ...
        if (it == attrs.end())
        {
            ...
            //if (md.objecttype == OTAI_OBJECT_TYPE_BUFFER_PROFILE &&
            //        (md.attrid == OTAI_BUFFER_PROFILE_ATTR_SHARED_DYNAMIC_TH ||
            //         (md.attrid == OTAI_BUFFER_PROFILE_ATTR_SHARED_STATIC_TH)))
            //{
            //    auto pool_id_attr = otai_metadata_get_attr_by_id(OTAI_BUFFER_PROFILE_ATTR_POOL_ID, attr_count, attr_list);
            ...
            //    otai_object_id_t pool_id = pool_id_attr->value.oid;

            //    if (pool_id == OTAI_NULL_OBJECT_ID)
            //    {
            ...
            //     * Now extract OTAI_BUFFER_POOL_THRESHOLD_MODE attribute
            //     */

            //    otai_object_meta_key_t mk = { .objecttype = OTAI_OBJECT_TYPE_BUFFER_POOL, .objectkey = { .key = { .object_id = pool_id } } };

            //    auto pool_md = otai_metadata_get_attr_metadata(OTAI_OBJECT_TYPE_BUFFER_POOL, OTAI_BUFFER_POOL_ATTR_THRESHOLD_MODE);

            //    auto prev = get_object_previous_attr(mk, *pool_md);

            //    otai_buffer_pool_threshold_mode_t mode;

            //    if (prev == NULL)
            //    {
            //        mode = (otai_buffer_pool_threshold_mode_t)pool_md->defaultvalue->s32;
            //    }
            //    else
            //    {
            //        mode = (otai_buffer_pool_threshold_mode_t)prev->getOtaiAttr()->value.s32;
            //    }

            //    if ((mode == OTAI_BUFFER_POOL_THRESHOLD_MODE_DYNAMIC && md.attrid == OTAI_BUFFER_PROFILE_ATTR_SHARED_DYNAMIC_TH) ||
            //            (mode == OTAI_BUFFER_POOL_THRESHOLD_MODE_STATIC && md.attrid == OTAI_BUFFER_PROFILE_ATTR_SHARED_STATIC_TH))
            //    {
            ...
            //if (md.attrid == OTAI_ACL_TABLE_ATTR_FIELD_ACL_RANGE_TYPE && md.objecttype == OTAI_OBJECT_TYPE_ACL_TABLE)
            //{
            //    /*
            //     * TODO Remove in future. Workaround for range type which in
            //     * headers was marked as mandatory by mistake, and we need to
            //     * wait for next OTAI integration to pull this change in.
            //     */
        ...
        _In_ const otai_attribute_t *attr)
{
    ...
    otai_object_id_t switch_id = OTAI_NULL_OBJECT_ID;
    ...
    switch (md.attrvaluetype)
    {
        ...
        case OTAI_ATTR_VALUE_TYPE_OBJECT_ID:

            {
                ...
                //if (md.objecttype == OTAI_OBJECT_TYPE_SCHEDULER_GROUP &&
                //        md.attrid == OTAI_SCHEDULER_GROUP_ATTR_SCHEDULER_PROFILE_ID &&
                //        value.oid == OTAI_NULL_OBJECT_ID)
                //{
        ...
        case OTAI_ATTR_VALUE_TYPE_OBJECT_LIST:
            ...
            {
                otai_status_t status = meta_generic_validation_objlist(md, switch_id, value.objlist.count, value.objlist.list);
        ...
        //        otai_status_t status = meta_generic_validation_objlist(md, switch_id, 1, &value.aclfield.data.oid);
        ...
        //        otai_status_t status = meta_generic_validation_objlist(md, switch_id, value.aclfield.data.objlist.count, value.aclfield.data.objlist.list);
        ...
        //        otai_status_t status = meta_generic_validation_objlist(md, switch_id, 1, &value.aclaction.parameter.oid);
        ...
        //        otai_status_t status = meta_generic_validation_objlist(md, switch_id, value.aclaction.parameter.objlist.count, value.aclaction.parameter.objlist.list);
    ...
    for (uint32_t idx = 0; idx < attr_count; ++idx)
    {
        ...
        switch (md.attrvaluetype)
        {
            ...
            //    break;
                ...
                // case OTAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_UINT8_LIST: (2 lists)
        ...
        _In_ const otai_object_id_t* list)
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        otai_object_id_t query_switch_id = switchIdQuery(oid);
        ...
        _In_ otai_object_id_t switch_id)
{
    ...
    for (size_t j = 0; j < info->structmemberscount; ++j)
    {
        ...
        otai_object_id_t oid_switch_id = switchIdQuery(oid);
...
otai_object_id_t Meta::meta_extract_switch_id(
        _In_ const otai_object_meta_key_t& meta_key,
        _In_ otai_object_id_t switch_id)
{
    SWSS_LOG_ENTER();
        ...
        _In_ const otai_attribute_t *attr_list)
{
     ...
     * port, then those object will be removed automatically by vendor OTAI, and
    ...
    //const otai_object_id_t port_id = meta_key.objectkey.key.object_id;

    //for (uint32_t idx = 0; idx < attr_count; ++idx)
    //{
    //    const otai_attribute_t& attr = attr_list[idx];

    //    auto& md = *otai_metadata_get_attr_metadata(meta_key.objecttype, attr.id);

    //    switch (md.attrid)
    //    {
    //        case OTAI_PORT_ATTR_QOS_QUEUE_LIST:
    //        case OTAI_PORT_ATTR_QOS_SCHEDULER_GROUP_LIST:
    //        case OTAI_PORT_ATTR_INGRESS_PRIORITY_GROUP_LIST:
        ...
        _In_ const otai_attribute_t *attr_list)
{
    ...
    if (meta_key.objecttype == OTAI_OBJECT_TYPE_LINECARD)
    {
        auto attr = otai_metadata_get_attr_by_id(OTAI_LINECARD_ATTR_INIT_LINECARD, attr_count, attr_list);
    ...
    else
    {
        ...
        do
        {
            ...
            if (meta_key.objecttype != OTAI_OBJECT_TYPE_LINECARD)
            {
                ...
                otai_object_id_t query_switch_id = switchIdQuery(meta_key.objectkey.key.object_id);
        ...
        _In_ const otai_attribute_t *attr)
{
    ...
    switch (md.attrvaluetype)
    {
        ...
        //        {
        //            // decrease previous if it was set
        //            if (prev->getOtaiAttr()->value.aclfield.enable)
        //                m_oids.objectReferenceDecrement(prev->getOtaiAttr()->value.aclfield.data.oid);
        //        }
        ...
        //            // decrease previous if it was set
        //            if (prev->getOtaiAttr()->value.aclfield.enable)
        //                m_oids.objectReferenceDecrement(prev->getOtaiAttr()->value.aclfield.data.objlist);
        //        }
        ...
        //        {
        //            // decrease previous if it was set
        //            if (prev->getOtaiAttr()->value.aclaction.enable)
        //                m_oids.objectReferenceDecrement(prev->getOtaiAttr()->value.aclaction.parameter.oid);
        //        }
        ...
        //            // decrease previous if it was set
        //            if (prev->getOtaiAttr()->value.aclaction.enable)
        //                m_oids.objectReferenceDecrement(prev->getOtaiAttr()->value.aclaction.parameter.objlist);
        //        }
...
//static const otai_mac_t zero_mac = { 0, 0, 0, 0, 0, 0 };
//
//void Meta::meta_otai_on_fdb_flush_event_consolidated(
...
//
//    auto bpid = otai_metadata_get_attr_by_id(OTAI_FDB_ENTRY_ATTR_BRIDGE_PORT_ID, data.attr_count, data.attr);
//    auto type = otai_metadata_get_attr_by_id(OTAI_FDB_ENTRY_ATTR_TYPE, data.attr_count, data.attr);
//
//    if (type == NULL)
//    {
//        SWSS_LOG_ERROR("FATAL: fdb flush notification don't contain OTAI_FDB_ENTRY_ATTR_TYPE attribute! bug!, no entries were flushed in local DB!");
//        return;
//    }
//
//    SWSS_LOG_NOTICE("processing consolidated fdb flush event of type: %s",
//            otai_metadata_get_fdb_entry_type_name((otai_fdb_entry_type_t)type->value.s32));
//
//    std::vector<otai_object_meta_key_t> toremove;
//
//    auto fdbEntries = m_otaiObjectCollection.getObjectsByObjectType(OTAI_OBJECT_TYPE_FDB_ENTRY);
//
//    for (auto& fdb: fdbEntries)
//    {
//        auto fdbTypeAttr = fdb->getAttr(OTAI_FDB_ENTRY_ATTR_TYPE);
//
//        if (!fdbTypeAttr)
//        {
//            SWSS_LOG_ERROR("FATAL: missing OTAI_FDB_ENTRY_ATTR_TYPE on %s! bug! skipping flush",
//                    otai_serialize_object_meta_key(fdb->getMetaKey()).c_str());
//            continue;
//        }
//
//        if (fdbTypeAttr->getOtaiAttr()->value.s32 != type->value.s32)
//        {
...
//        if (bpid != NULL && bpid->value.oid != OTAI_NULL_OBJECT_ID)
//        {
//            auto bpidAttr = fdb->getAttr(OTAI_FDB_ENTRY_ATTR_BRIDGE_PORT_ID);
//
//            if (!bpidAttr)
//            {
...
//            }
//
//            if (bpidAttr->getOtaiAttr()->value.oid != bpid->value.oid)
//            {
...
//
//        if (data.fdb_entry.bv_id != OTAI_NULL_OBJECT_ID)
//        {
...
//
//        SWSS_LOG_INFO("removing %s", otai_serialize_object_meta_key(meta_key_fdb).c_str());
//
//        // since meta_generic_validation_post_remove also modifies m_otaiObjectCollection
        ...
        _In_ otai_object_id_t oid)
{
     ...
     * vendor bug in OTAI and we should also see warnings or errors reported
     * from syncd in logs.
     */
...
//void Meta::meta_otai_on_fdb_event_single(
//        _In_ const otai_fdb_event_notification_data_t& data)
//{
//    SWSS_LOG_ENTER();
//
//    const otai_object_meta_key_t meta_key_fdb = { .objecttype = OTAI_OBJECT_TYPE_FDB_ENTRY, .objectkey = { .key = { .fdb_entry = data.fdb_entry } } };
//
//    /*
...
//     * or whether there maybe some bug in vendor OTAI and for example is sending
...
//    {
//        auto meta = otai_metadata_get_attr_metadata(OTAI_OBJECT_TYPE_FDB_ENTRY, data.attr[i].id);
//
//        if (meta == NULL)
//        {
...
//
//            {
//                otai_attribute_t *list = data.attr;
//                uint32_t count = data.attr_count;
//
//                otai_attribute_t local[2]; // 2 for port id and type
//
//                if (count == 1)
//                {
...
//                    local[1].id = OTAI_FDB_ENTRY_ATTR_TYPE;
//                    local[1].value.s32 = OTAI_FDB_ENTRY_TYPE_DYNAMIC; // assume learned entries are always dynamic
//
//                    list = local;
//                    count = 2; // now we added type
//                }
//
//                otai_status_t status = meta_generic_validation_create(meta_key_fdb, data.fdb_entry.switch_id, count, list);
//
//                if (status == OTAI_STATUS_SUCCESS)
//                {
...
//            for (uint32_t i = 0; i < data.attr_count; i++)
//            {
//                const otai_attribute_t& attr = data.attr[i];
//
//                otai_status_t status = meta_generic_validation_set(meta_key_fdb, &attr);
//
//                if (status != OTAI_STATUS_SUCCESS)
//                {
...
//}
//
//void Meta::meta_otai_on_fdb_event(
//        _In_ uint32_t count,
//        _In_ const otai_fdb_event_notification_data_t *data)
//{
//    SWSS_LOG_ENTER();
...
//    for (uint32_t i = 0; i < count; ++i)
//    {
//        meta_otai_on_fdb_event_single(data[i]);
//    }
//}

//void Meta::meta_otai_on_nat_event_single(
//        _In_ const otai_nat_event_notification_data_t& data)
//{
//    SWSS_LOG_ENTER();
//
//    const otai_object_meta_key_t meta_key_nat = { .objecttype = OTAI_OBJECT_TYPE_NAT_ENTRY, .objectkey = { .key = { .nat_entry = data.nat_entry } } };
//
//    switch (data.event_type)
//    {
//        case OTAI_NAT_EVENT_AGED:
//
//            if (!m_otaiObjectCollection.objectExists(meta_key_nat))
//            {
//                SWSS_LOG_WARN("object key %s doesn't exist but received AGED event",
...
//}
//
//void Meta::meta_otai_on_nat_event(
//        _In_ uint32_t count,
//        _In_ const otai_nat_event_notification_data_t *data)
//{
//    SWSS_LOG_ENTER();
...
//    for (uint32_t i = 0; i < count; ++i)
//    {
//        meta_otai_on_nat_event_single(data[i]);
//    }
//}

//void Meta::meta_otai_on_port_host_tx_ready_change(
//                    _In_ otai_object_id_t port_id,
//                    _In_ otai_object_id_t switch_id,
...
//            &otai_metadata_enum_otai_port_host_tx_ready_status_t,
//            host_tx_ready_status))
//    {
//        SWSS_LOG_WARN("port host_tx_ready value (%d) not found in otai_port_host_tx_ready_status_t. Dropping the notification",
//                host_tx_ready_status);
//
//        return;
//    }
...
//                otai_serialize_object_id(port_id).c_str());
//
//        otai_object_meta_key_t host_tx_ready_key = { .objecttype = ot, .objectkey = { .key = { .object_id = port_id } } };
//        m_oids.objectReferenceInsert(port_id);
//
//        if (!m_otaiObjectCollection.objectExists(host_tx_ready_key))
//        {
//            m_otaiObjectCollection.createObject(host_tx_ready_key);
//        }
//    }
//}


//void Meta::meta_otai_on_switch_state_change(
//        _In_ otai_object_id_t switch_id,
...
//
//    if (!otai_metadata_get_enum_value_name(
//                &otai_metadata_enum_otai_switch_oper_status_t,
//                switch_oper_status))
//    {
//        SWSS_LOG_WARN("switch oper status value (%d) not found in otai_switch_oper_status_t",
//                switch_oper_status);
//    }
//
...
//    {
//        SWSS_LOG_WARN("switch_id %s is of type %s, but expected OTAI_OBJECT_TYPE_LINECARD",
//                otai_serialize_object_id(switch_id).c_str(),
//                otai_serialize_object_type(ot).c_str());
//
//        return;
//    }
//
//    otai_object_meta_key_t switch_meta_key = { .objecttype = ot , .objectkey = { .key = { .object_id = switch_id } } };
//
//    if (!m_otaiObjectCollection.objectExists(switch_meta_key))
//    {
...
//void Meta::meta_otai_on_switch_asic_sdk_health_event(
//        _In_ otai_object_id_t switch_id,
...
//
//    if (!otai_metadata_get_enum_value_name(
//            &otai_metadata_enum_otai_switch_asic_sdk_health_severity_t,
//            severity))
//    {
//        SWSS_LOG_WARN("Switch ASIC/SDK health event severity value (%d) not found in otai_switch_asic_sdk_health_severity_t",
//                      severity);
//    }
//
//    if (!otai_metadata_get_enum_value_name(
//            &otai_metadata_enum_otai_switch_asic_sdk_health_category_t,
//            category))
//    {
//        SWSS_LOG_WARN("Switch ASIC/SDK health event category value (%d) not found in otai_switch_asic_sdk_health_severity_t",
//                      category);
//    }
//
//    auto ot = objectTypeQuery(switch_id);
...
void Meta::meta_otai_on_switch_shutdown_request(
        _In_ otai_object_id_t switch_id)
{
    SWSS_LOG_ENTER();
...
//void Meta::meta_otai_on_port_state_change_single(
...
//    switch (ot)
//    {
//        // TODO hardcoded types, must advance OTAI repository commit to get metadata for this
//        case OTAI_OBJECT_TYPE_PORT:
//        case OTAI_OBJECT_TYPE_BRIDGE_PORT:
//        case OTAI_OBJECT_TYPE_LAG:
//
//            valid = true;
//            break;
//
...
//                    otai_serialize_object_id(data.port_id).c_str(),
//                    otai_serialize_object_type(ot).c_str());
//            break;
//    }
//
...
//                otai_serialize_object_id(data.port_id).c_str());
//
//        otai_object_meta_key_t key = { .objecttype = ot, .objectkey = { .key = { .object_id = data.port_id } } };
//
...
//
//    if (!otai_metadata_get_enum_value_name(
//                &otai_metadata_enum_otai_port_oper_status_t,
//                data.port_state))
//    {
//        SWSS_LOG_WARN("port_state value (%d) not found in otai_port_oper_status_t",
//                data.port_state);
//    }
//}
//
//void Meta::meta_otai_on_port_state_change(
//        _In_ uint32_t count,
//        _In_ const otai_port_oper_status_notification_t *data)
//{
//    SWSS_LOG_ENTER();
...
//    for (uint32_t i = 0; i < count; ++i)
//    {
//        meta_otai_on_port_state_change_single(data[i]);
//    }
//}

//void Meta::meta_otai_on_queue_pfc_deadlock_notification_single(
...
//                    otai_serialize_object_id(data.queue_id).c_str(),
//                    otai_serialize_object_type(ot).c_str());
//            break;
//    }
//
...
//                otai_serialize_object_id(data.queue_id).c_str());
//
//        otai_object_meta_key_t key = { .objecttype = ot, .objectkey = { .key = { .object_id = data.queue_id } } };
//
...
//}
//
//void Meta::meta_otai_on_queue_pfc_deadlock_notification(
//        _In_ uint32_t count,
//        _In_ const otai_queue_deadlock_notification_data_t *data)
//{
//    SWSS_LOG_ENTER();
...
//    for (uint32_t i = 0; i < count; ++i)
//    {
//        meta_otai_on_queue_pfc_deadlock_notification_single(data[i]);
//    }
//}

//void Meta::meta_otai_on_bfd_session_state_change_single(
...
//    {
//        // TODO hardcoded types, must advance OTAI repository commit to get metadata for this
//        case OTAI_OBJECT_TYPE_BFD_SESSION:
//
//            valid = true;
//            break;
//
...
//                    otai_serialize_object_id(data.bfd_session_id).c_str(),
//                    otai_serialize_object_type(ot).c_str());
//            break;
//    }
//
...
//                otai_serialize_object_id(data.bfd_session_id).c_str());
//
//        otai_object_meta_key_t key = { .objecttype = ot, .objectkey = { .key = { .object_id = data.bfd_session_id } } };
//
...
//}
//
//void Meta::meta_otai_on_bfd_session_state_change(
//        _In_ uint32_t count,
//        _In_ const otai_bfd_session_state_notification_t *data)
//{
//    SWSS_LOG_ENTER();
...
//    if (count && data == NULL)
//    {
//        SWSS_LOG_ERROR("otai_bfd_session_state_notification_t pointer is NULL but count is %u", count);
//        return;
//    }
//
//    for (uint32_t i = 0; i < count; ++i)
//    {
//        meta_otai_on_bfd_session_state_change_single(data[i]);
//    }
//}

//void Meta::meta_otai_on_twamp_session_event_single(
...
//    {
//        // TODO hardcoded types, must advance OTAI repository commit to get metadata for this
//        case OTAI_OBJECT_TYPE_TWAMP_SESSION:
//
//            valid = true;
//            break;
//
...
//                    otai_serialize_object_id(data.twamp_session_id).c_str(),
//                    otai_serialize_object_type(ot).c_str());
//            break;
//    }
//
...
//    {
//        if (!otai_metadata_get_enum_value_name(&otai_metadata_enum_otai_twamp_session_stat_t, data.session_stats.counters_ids[idx]))
//        {
//            SWSS_LOG_ERROR("value %d is not in range on otai_twamp_session_stat_t ", data.session_stats.counters_ids[idx]);
//
//            return;
//        }
//    }
...
//                otai_serialize_object_id(data.twamp_session_id).c_str());
//
//        otai_object_meta_key_t key = { .objecttype = (otai_object_type_t)ot, .objectkey = { .key = { .object_id = data.twamp_session_id } } };
//
...
//
//    if (!otai_metadata_get_enum_value_name(
//                &otai_metadata_enum_otai_twamp_session_state_t,
//                data.session_state))
//    {
//        SWSS_LOG_WARN("session_state value (%d) not found otai_twamp_session_state_t",
//                data.session_state);
//    }
//}
//
//void Meta::meta_otai_on_twamp_session_event(
//        _In_ uint32_t count,
//        _In_ const otai_twamp_session_event_notification_data_t *data)
//{
//    SWSS_LOG_ENTER();
...
//    if (count && data == NULL)
//    {
//        SWSS_LOG_ERROR("otai_twamp_session_event_notification_data_t pointer is NULL but count is %u", count);
//        return;
//    }
//
//    for (uint32_t i = 0; i < count; ++i)
//    {
//        meta_otai_on_twamp_session_event_single(data[i]);
//    }
//}
        ...
        _In_ const swss::TableDump& dump)
{
    ...
    for (const auto &key: dump)
    {
        ...
        OtaiAttributeList alist(mk.objecttype, hash, false);
        ...
        if (!m_otaiObjectCollection.objectExists(mk))
            m_otaiObjectCollection.createObject(mk);

        auto info = otai_metadata_get_object_type_info(mk.objecttype);
        ...
        for (uint32_t idx = 0; idx < attr_count; ++idx)
        {
            ...
            auto mdp = otai_metadata_get_attr_metadata(mk.objecttype, attr->id);
            ...
            m_otaiObjectCollection.setObjectAttr(mk, md, attr);
        }

        if (haskeys)
        {
            auto mKey = otai_serialize_object_meta_key(mk);
