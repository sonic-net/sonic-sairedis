...
class RouteConfiger(object):
    ...
    def create_router_interface(self, net_interface: route_item, virtual_router=None, reuse=True, is_bridge=False):
        ...
        if not reuse or not (net_interface.rif_list and len(net_interface.rif_list) != 0):
            ...
            self._vpp_ut_assign_rif_ips(net_interface)
        return net_interface.rif_list[-1]

    def _vpp_ut_assign_rif_ips(self, net_interface):
        """VPP UT harness: assign kernel/VPP connected IPs for LAG/SVI RIFs."""
        try:
            from config import vpp_ut_support
        except ImportError:
            return
        if not vpp_ut_support.enabled():
            return
        if isinstance(net_interface, Lag):
            try:
                ...
                return
            vpp_ut_support.assign_lag_rif_ips(lag_index)
        elif isinstance(net_interface, Vlan):
            vpp_ut_support.assign_svi_rif_ips(net_interface.vlan_id)
