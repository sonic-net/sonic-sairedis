...
std::map<sai_object_type_t, std::vector<SaiChildRelation>> sai_child_relation_defs = {
    ...
    {SAI_OBJECT_TYPE_PORT,             {{SAI_OBJECT_TYPE_ACL_TABLE_GROUP, SAI_PORT_ATTR_INGRESS_ACL, SAI_ATTR_VALUE_TYPE_OBJECT_ID},
                                        {SAI_OBJECT_TYPE_ACL_TABLE_GROUP, SAI_PORT_ATTR_EGRESS_ACL, SAI_ATTR_VALUE_TYPE_OBJECT_ID}}},
    {SAI_OBJECT_TYPE_LAG,              {{SAI_OBJECT_TYPE_ACL_TABLE_GROUP, SAI_LAG_ATTR_INGRESS_ACL, SAI_ATTR_VALUE_TYPE_OBJECT_ID},
                                        {SAI_OBJECT_TYPE_ACL_TABLE_GROUP, SAI_LAG_ATTR_EGRESS_ACL, SAI_ATTR_VALUE_TYPE_OBJECT_ID}}}
};
            ...
            _In_ sai_attr_id_t link_attr_id) const
{
    ...
    status = get_attr(attr);
    if (status != SAI_STATUS_SUCCESS) {
        SWSS_LOG_ERROR("Failed to get attribute %s from object %s", get_attr_name(link_attr_id), m_id.c_str());
        return linked_objs;
    }
    sai_object_list_t& linked_obj_list = attr.value.objlist;
    for (uint32_t i = 0; i < linked_obj_list.count; i++) {
