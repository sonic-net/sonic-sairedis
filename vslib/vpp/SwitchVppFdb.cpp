        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    SWSS_LOG_NOTICE("FDB: flush done (mode=%d), m_vpp_fdb_entries=%zu",
                    mode, m_vpp_fdb_entries.size());
...
{
    SWSS_LOG_DEBUG("FDB: invalidating all %zu m_vpp_fdb_entries", m_vpp_fdb_entries.size());
...
void SwitchVpp::vpp_fdb_entries_invalidate_by_bd(uint32_t bd_id)
{
    ...
    SWSS_LOG_DEBUG("FDB: invalidated by bd_id=%u, removed %zu entries, %zu remain",
...
void SwitchVpp::vpp_fdb_entries_invalidate_by_port(sai_object_id_t port_id)
{
     ...
     * TODO: store port_id alongside sw_if_index in m_vpp_fdb_entries to make
     *       this O(N) with a simple OID comparison instead of VPP API calls. */
    size_t before = m_vpp_fdb_entries.size();
    for (auto it = m_vpp_fdb_entries.begin(); it != m_vpp_fdb_entries.end(); )
    {
    ...
    SWSS_LOG_DEBUG("FDB: invalidated by port_id=%s, removed %zu entries, %zu remain",
                   sai_serialize_object_id(port_id).c_str(),
 ...
 * VPP sw_if_index ->
 * VPP hw interface name (e.g. "bobm0") ->
 * Linux tap/SONiC port name (e.g. "Ethernet4") ->
 * SAI port OID (e.g. oid:0x...)
...
void SwitchVpp::generateFdbLearnedOrMoveEvent(const VppFdbKey &key, uint32_t sw_if_index, sai_fdb_event_t event_type)
{
    ...
    {
        SWSS_LOG_WARN("FDB: cannot resolve port OID for sw_if_index %u", sw_if_index);
        return;
    }
    ...
    {
        SWSS_LOG_WARN("FDB: bv_id or bridge_port not found for sw_if_index %u bd %u",
    ...
    if (status != SAI_STATUS_SUCCESS)
    {
        SWSS_LOG_WARN("FDB: %s failed for %s: %s",
    ...
    SWSS_LOG_INFO("FDB: notified %s for MAC %02x:%02x:%02x:%02x:%02x:%02x bd %u sw_if_index %u",
...
void SwitchVpp::generateFdbAgedEvent(const VppFdbKey &key)
{
    ...
    {
        SWSS_LOG_WARN("FDB: entry not found in m_fdb_info_set for bd %u", key.bd_id);
        return;
    }
