    ...
    if (attr->id == SAI_TAM_TEL_TYPE_ATTR_STATE)
    {
        switch (attr->value.s32)
        {
            ...
            case SAI_TAM_TEL_TYPE_STATE_START_STREAM:
            {
                ...
                if (num_counters > 0)
                {
                    // Read IPFIX template from SAI attribute
                    // (generated by refresh_tam_tel_ipfix_templates on CREATE_CONFIG)
                    std::vector<uint8_t> ipfix_template;
                    sai_attribute_t tmpl_attr;
                    tmpl_attr.id = SAI_TAM_TEL_TYPE_ATTR_IPFIX_TEMPLATES;
                    std::vector<uint8_t> tmpl_buf(4096);
                    tmpl_attr.value.u8list.count = static_cast<uint32_t>(tmpl_buf.size());
                    tmpl_attr.value.u8list.list = tmpl_buf.data();
                    if (get(SAI_OBJECT_TYPE_TAM_TEL_TYPE,
                            sai_serialize_object_id(tam_tel_type_id),
                            1, &tmpl_attr) == SAI_STATUS_SUCCESS)
                    {
                        ipfix_template.assign(tmpl_buf.begin(),
                                              tmpl_buf.begin() + tmpl_attr.value.u8list.count);
                        SWSS_LOG_NOTICE("Read IPFIX template (%u bytes) for STEL stream",
                                        tmpl_attr.value.u8list.count);
                    }
                    else
                    {
                        SWSS_LOG_WARN("Could not read IPFIX template, data-only stream");
                    }

                    startStelStream(poll_interval_us, template_id, num_counters, ipfix_template);
                }
                break;
            }
