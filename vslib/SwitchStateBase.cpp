...
sai_status_t SwitchStateBase::setLagMember(
        _In_ sai_object_id_t lagMemberId,
        _In_ const sai_attribute_t* attr)
{
    SWSS_LOG_ENTER();

    if (attr && m_switchConfig->m_useTapDevice &&
        (attr->id == SAI_LAG_MEMBER_ATTR_EGRESS_DISABLE ||
         attr->id == SAI_LAG_MEMBER_ATTR_INGRESS_DISABLE))
    {
        bool disable = attr->value.booldata;

        // Get the port_id from this LAG member
        sai_attribute_t port_attr;
        port_attr.id = SAI_LAG_MEMBER_ATTR_PORT_ID;

        sai_status_t status = get(SAI_OBJECT_TYPE_LAG_MEMBER, lagMemberId, 1, &port_attr);

        if (status != SAI_STATUS_SUCCESS)
        {
            SWSS_LOG_ERROR("Failed to get port_id for LAG member %s",
                    sai_serialize_object_id(lagMemberId).c_str());
        }
        else
        {
            sai_object_id_t portId = port_attr.value.oid;
            std::string tapname;

            if (getTapNameFromPortId(portId, tapname))
            {
                bool isEgress = (attr->id == SAI_LAG_MEMBER_ATTR_EGRESS_DISABLE);

                if (isEgress)
                {
                    // For egress disable: use a noqueue qdisc to silently drop all
                    // outgoing packets. This prevents the DUT from sending BGP
                    // keepalives (and any other traffic) out this LAG member without
                    // affecting link state, LACP, or team membership.
                    std::string cmd;
                    if (disable)
                    {
                        cmd = "/sbin/tc qdisc replace dev " + tapname + " root handle 1: pfifo limit 0";
                    }
                    else
                    {
                        cmd = "/sbin/tc qdisc del dev " + tapname + " root 2>/dev/null; true";
                    }

                    int ret = std::system(cmd.c_str());

                    SWSS_LOG_NOTICE("%s egress on LAG member port %s (ret=%d)",
                            disable ? "Blocking" : "Unblocking",
                            tapname.c_str(), ret);
                }
                else
                {
                    // For ingress disable: use tc ingress + filter to drop all
                    // incoming packets so the DUT doesn't receive anything from
                    // this LAG member.
                    std::string cmd;
                    if (disable)
                    {
                        cmd = "/sbin/tc qdisc add dev " + tapname + " ingress 2>/dev/null; "
                              "/sbin/tc filter add dev " + tapname +
                              " parent ffff: protocol all u32 match u32 0 0 action drop";
                    }
                    else
                    {
                        cmd = "/sbin/tc qdisc del dev " + tapname + " ingress 2>/dev/null; true";
                    }

                    int ret = std::system(cmd.c_str());

                    SWSS_LOG_NOTICE("%s ingress on LAG member port %s (ret=%d)",
                            disable ? "Blocking" : "Unblocking",
                            tapname.c_str(), ret);
                }
            }
            ...
            {
                SWSS_LOG_WARN("Could not find tap device for port %s",
                        sai_serialize_object_id(portId).c_str());
            }
        }
    }
