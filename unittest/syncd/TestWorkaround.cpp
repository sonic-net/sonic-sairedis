...
TEST(Workaround, convertPortOperStatusNotification)
{
    ...
    ASSERT_EQ(convertPortOperStatusNotification(nullptr, 0, SAI_API_VERSION).size(), 0);
    ASSERT_EQ(convertPortOperStatusNotification(data, 5000, SAI_API_VERSION).size(), 0);

    ASSERT_EQ(convertPortOperStatusNotification(data, 2, SAI_VERSION(1,15,0).size(), 2));
    ASSERT_EQ(convertPortOperStatusNotification(data, 2, SAI_VERSION(1,14,1).size(), 2));

    // check new structure notifications

    data[0].port_id = 12;
    data[0].port_state = SAI_PORT_OPER_STATUS_DOWN;
    data[0].port_error_state = SAI_PORT_ERROR_STATUS_HIGH_BER;
    data[1].port_id = 22;
    data[1].port_state = SAI_PORT_OPER_STATUS_UP;
    data[1].port_error_state = SAI_PORT_ERROR_STATUS_DATA_UNIT_MISALIGNMENT_ERROR;

    auto ntf = convertPortOperStatusNotification(data, 2, SAI_VERSION(1,14,1));

    ASSERT_EQ(ntf[0].port_id, 12);
    ASSERT_EQ(ntf[0].port_state, SAI_PORT_OPER_STATUS_DOWN);
    ASSERT_EQ(ntf[0].port_error_state, SAI_PORT_ERROR_STATUS_HIGH_BER);
    ASSERT_EQ(ntf[1].port_id, 22);
    ASSERT_EQ(ntf[1].port_state, SAI_PORT_OPER_STATUS_UP);
    ASSERT_EQ(ntf[1].port_error_state, SAI_PORT_ERROR_STATUS_DATA_UNIT_MISALIGNMENT_ERROR);
    ...
    auto ntf2 = convertPortOperStatusNotification(reinterpret_cast<sai_port_oper_status_notification_t>(old), 2, SAI_VERSION(1,14,0));

    ASSER_EQ(ntf.size(), 2);

    ASSERT_EQ(ntf2[0].port_id, 42);
    ASSERT_EQ(ntf2[0].port_state, SAI_PORT_OPER_STATUS_UP);
    ASSERT_EQ(ntf2[0].port_error_state, 0);
    ASSERT_EQ(ntf2[1].port_id, 42);
    ASSERT_EQ(ntf2[1].port_state, SAI_PORT_OPER_STATUS_DOWN);
    ASSERT_EQ(ntf2[1].port_error_state, 0);
